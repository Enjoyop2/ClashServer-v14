...
namespace ClashofClans.Logic.Manager
{
	public class GameObjectManager
	{
		...
		{
			int i = Buildings.FindIndex(building => building.BuildingData.GetGlobalId() == 1000001);
		...
		{
			int i = Buildings2.FindIndex(building => building.BuildingData.GetGlobalId() == 1000034);
		...
		{
			foreach (Building building in Buildings) building.FastForward(seconds);
			foreach (Building building in Buildings2) building.FastForward(seconds);
			foreach (Obstacle obstacle in Obstacles) obstacle.FastForward(seconds);
			foreach (Obstacle obstacle in Obstacles2) obstacle.FastForward(seconds);
			foreach (Trap trap in Traps) trap.FastForward(seconds);
			foreach (Trap trap in Traps2) trap.FastForward(seconds);
			foreach (VillageObject villageObject in VillageObjects) villageObject.FastForward(seconds);
			foreach (VillageObject villageObject in VillageObjects2) villageObject.FastForward(seconds);
		}
		...
		public void Tick()
		{
			foreach (Building building in Buildings) building.Tick();
			foreach (Building building in Buildings2) building.Tick();
			...
			foreach (Obstacle obstacle in Obstacles2.ToArray())
			{
				obstacle.Tick();
			...
			foreach (Trap trap in Traps) trap.Tick();
			foreach (Trap trap in Traps2) trap.Tick();

			foreach (VillageObject villageObject in VillageObjects) villageObject.Tick();
			foreach (VillageObject villageObject in VillageObjects2) villageObject.Tick();
		}
		...
		public string Save()
		{
			...
			foreach (Building building in Buildings) buildings.Add(building.Save());
			json.Add("buildings", buildings);

			JArray obstacles = new JArray();
			foreach (Obstacle obstacle in Obstacles) obstacles.Add(obstacle.Save());
			json.Add("obstacles", obstacles);

			JArray decos = new JArray();
			foreach (Deco deco in Decos) decos.Add(deco.Save());
			json.Add("decos", decos);

			JArray traps = new JArray();
			foreach (Trap trap in Traps) traps.Add(trap.Save());
			json.Add("traps", traps);

			JArray villageObjects = new JArray();
			foreach (VillageObject villageObject in VillageObjects) villageObjects.Add(villageObject.Save());
			json.Add("vobjs", villageObjects);

			// Builderbase
			JArray buildings2 = new JArray();
			foreach (Building building in Buildings2) buildings2.Add(building.Save());
			json.Add("buildings2", buildings2);

			JArray obstacles2 = new JArray();
			foreach (Obstacle obstacle in Obstacles2) obstacles2.Add(obstacle.Save());
			json.Add("obstacles2", obstacles2);

			JArray decos2 = new JArray();
			foreach (Deco deco in Decos2) decos2.Add(deco.Save());
			json.Add("decos2", decos2);

			JArray traps2 = new JArray();
			foreach (Trap trap in Traps2) traps2.Add(trap.Save());
			json.Add("traps2", traps2);

			JArray villageObjects2 = new JArray();
			foreach (VillageObject villageObject in VillageObjects2) villageObjects2.Add(villageObject.Save());
			json.Add("vobjs2", villageObjects2);

			JArray selectedBackground = new JArray();
			foreach (int bg in SelectedBackgrounds) selectedBackground.Add(bg);
		...
		public void LoadJson(string json)
		{
			...
			{
				Buildings.Clear();
				foreach (JToken jToken in j["buildings"])
				{
			...
			{
				Obstacles.Clear();
				foreach (JToken jToken in j["obstacles"])
				{
					JObject obj = (JObject)jToken;
					Obstacle obstacle = new Obstacle(Home)
					{
			...
			{
				Decos.Clear();
				foreach (JToken jToken in j["decos"])
				{
					JObject obj = (JObject)jToken;
			...
			{
				Traps.Clear();
				foreach (JToken jToken in j["traps"])
				{
					JObject obj = (JObject)jToken;
			...
			if (j.ContainsKey("vobjs"))
			{
				...
				foreach (JToken jToken in j["vobjs"])
				{
					JObject obj = (JObject)jToken;
					VillageObject villageObject = new VillageObject(Home)
					{
			...
			{
				Buildings2.Clear();
				foreach (JToken jToken in j["buildings2"])
				{
					JObject obj = (JObject)jToken;
					Building building2 = new Building(Home)
					{
			...
			{
				Obstacles2.Clear();
				foreach (JToken jToken in j["obstacles2"])
				{
					JObject obj = (JObject)jToken;
					Obstacle obstacle = new Obstacle(Home)
					{
			...
			{
				Decos2.Clear();
				foreach (JToken jToken in j["decos2"])
				{
					JObject obj = (JObject)jToken;
					Deco deco = new Deco(Home)
					{
			...
			{
				Traps2.Clear();
				foreach (JToken jToken in j["traps2"])
				{
					JObject obj = (JObject)jToken;
					Trap trap = new Trap(Home)
					{
			...
			if (j.ContainsKey("vobjs2"))
			{
				...
				foreach (JToken jToken in j["vobjs2"])
				{
					JObject obj = (JObject)jToken;
					VillageObject villageObject = new VillageObject(Home)
					{
