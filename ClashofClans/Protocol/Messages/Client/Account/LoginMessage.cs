...
namespace ClashofClans.Protocol.Messages.Client.Account
{
	public class LoginMessage : PiranhaMessage
	{
		...
		{
			Player player = await Resources.Players.Login(UserId, UserToken);
			...
			if (player != null)
			{
				...
				Logic.Clan.Alliance alliance = await Resources.Alliances.GetAllianceAsync(player.Home.AllianceInfo.Id);
