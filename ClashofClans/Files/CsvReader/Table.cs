	...
	public class Table
	{
		...
		public Table(string path)
		{
			...
			using (TextFieldParser reader = new TextFieldParser(path))
			{
				...
				foreach (string type in types) _types.Add(type);
				...
				while (!reader.EndOfData)
				{
					...
					for (int i = 0; i < _headers.Count; i++) _columns[i].Add(values[i]);
				}
			}
		}
		...
		{
			int index = _headers.IndexOf(name);
